...
public class LotroSoftSynthesizer implements AudioSynthesizer, ReferenceCountingDevice {
	...
	public Instrument[] getAvailableInstruments() {
		...
		synchronized (control_mutex) {
			ModelInstrument[] inslist_array = new ModelInstrument[availlist.values().size()];
		...
		synchronized (control_mutex) {
			try {
				...
				if (!line.isOpen()) {
					...
					// so we can close again in LotroSoftSynthesizer.close()
					sourceDataLine = line;
				}
				if (!line.isActive())
					line.start();
