 ...
 * have any questions.
 */
package com.digero.common.midi.synth;
...
public class SoftVoice extends VoiceStatus {
	...
	protected LotroSoftSynthesizer synthesizer;
	...
	public SoftVoice(LotroSoftSynthesizer synth) {
		synthesizer = synth;
	...
	protected void processControlLogic() {
		...
		if (audiostarted) {
			...
			/*
			 * filter_freq = 440.0 * Math.pow(2.0, ((co_filter_freq[0]) - 6900.0) / 1200.0);
			 */
			/*
			 * double velocity = co_noteon_velocity[0]; if(velocity < 0.5) filter_freq *= ((velocity * 2)*0.75 + 0.25);
			 */
			...
			/*
			 * float gain = (float) Math.pow(10, (-osc_attenuation + co_mixer_gain[0]) / 200.0);
			 */
	...
	protected void processAudioLogic(SoftAudioBuffer[] buffer) {
		...
		if (nrofchannels == 1) {
			...
			if (rightdry != null)
				mixAudioStream(rightdry, left, last_out_mixer_left, out_mixer_left);
		} else {
			mixAudioStream(leftdry, left, last_out_mixer_left, out_mixer_left);
			if (rightdry != null)
				mixAudioStream(rightdry, right, last_out_mixer_right, out_mixer_right);
			else
				mixAudioStream(leftdry, right, last_out_mixer_right, out_mixer_right);
		}
