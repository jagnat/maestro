...
public class SoftChannel implements MidiChannel, ModelDirectedPlayer {
	...
	private int bank;
	private int program;
	private LotroSoftSynthesizer synthesizer;
	...
	public SoftChannel(LotroSoftSynthesizer synth, int channel) {
	...
	private ModelConnectionBlock[] createModelConnections(ModelIdentifier sid, int[] destination, int[] range) {
		...
		for (int i = 0; i < destination.length; i++) {
			...
			if (d == 2) {
				...
				ModelConnectionBlock conn = new ModelConnectionBlock(new ModelSource(sid, mt), -960,
	...
	public void controlChange(int controller, int value) {
		...
		synchronized (control_mutex) {
			...
			case 65:
				...
				/*
				 * for (int i = 0; i < portamento_lastnote.length; i++) portamento_lastnote[i] = -1;
				 */
